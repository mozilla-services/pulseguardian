                ...
                for param in self.valid_opts[k]:
                    if param is None or input is None:
                        ...
                        if str(input).lower() == str(param):
                            return True
                raise ValueError(
                    "Invalid input for '%s' parameter. Options are: %s"
                    % (k, " ".join(["'%s'," % str(o) for o in self.valid_opts[k]]))
                )
            else:
                raise ValueError(
                    "Invalid parameter for '%s'. Params are: %s"
                    % (
                        self.__class__.__name__,
                        ", ".join(["'%s'" % p for p in self.valid_opts.keys()]),
                    )
                )
...
class CSP:
        ...
        if self.inputs is not None:
            for p, l in self.inputs.items():
                if p not in cspDefaultHeaders.keys() and p != "rewrite":
                    raise ValueError(
                        "Invalid parameter '%s'. Params are: %s"
    ...
    def update_policy(self, cspDefaultHeaders):
        ...
        if self.inputs is not None:
            for p, l in self.inputs.items():
                cspDefaultHeaders[p] = cspDefaultHeaders[p] + list(
                    set(self.inputs[p]) - set(cspDefaultHeaders[p])
                )
    ...
    def create_header(self):
        ...
        for p, array in self.inputs.items():
            csp[p] = " ".join(
                ["'%s'" % l if encapsulate.match(l) else l for l in array]
            )
