                ...
                for param in self.valid_opts[k]:
                    if param is None or input is None:
                        ...
                        if re.search(r'^' + param, str(input)):
                            return True
                    elif isinstance(param, bool) and isinstance(input, bool):
                        return True
                    elif isinstance(param, list) and isinstance(input, list):
                        return True
                    else:
                        if str(input).lower() == str(param):
                            return True
                raise ValueError("Invalid input for '%s' parameter. Options are: %s" % (
                    k, ' '.join(["'%s'," % str(o) for o in self.valid_opts[k]])))
            else:
                raise ValueError("Invalid parameter for '%s'. Params are: %s" % (
                    self.__class__.__name__, ', '.join(["'%s'" % p for p in self.valid_opts.keys()])))
    ...
    def create_header(self):
        ...
        for k, v in self.inputs.items():
            if v is None:
                ...
                lambda v: len(v) > 0, [_header_list.append(''.join(
                    ['pin-%s=%s' % (pink, pinv) for pink, pinv in pin.items()])) for pin in v]
            else:
                _header_list.append('%s=%s' % (k, str(v)))
        return {self.__class__.__name__.replace(
            '_', '-'): '; '.join(_header_list)}
...
class CSP:
    ...
    def check_valid(self, cspDefaultHeaders):
        if self.inputs is not None:
            for p, l in self.inputs.items():
                if p not in cspDefaultHeaders.keys() and p != 'rewrite':
                    raise ValueError("Invalid parameter '%s'. Params are: %s" % (
                        p, ', '.join(["'%s'" % p for p in cspDefaultHeaders.keys()])))
    ...
    def update_policy(self, cspDefaultHeaders):
        ...
        if self.inputs is not None:
            for p, l in self.inputs.items():
                cspDefaultHeaders[p] = cspDefaultHeaders[p] + \
                    list(set(self.inputs[p]) - set(cspDefaultHeaders[p]))
    ...
    def create_header(self):
        ...
        encapsulate = re.compile("|".join(['^self',
                                           '^none',
                                           '^unsafe-inline',
                                           '^unsafe-eval',
                                           r'^sha[\d]+-[\w=-]+',
        ...
        for p, array in self.inputs.items():
            csp[p] = ' '.join(["'%s'" %
                               l if encapsulate.match(l) else l for l in array])

        return {self.header: '; '.join(
            ['%s %s' % (k, v) for k, v in csp.items() if v != ''])}
